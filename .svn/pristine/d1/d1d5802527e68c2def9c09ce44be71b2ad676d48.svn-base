...
//				$(".pay-amount").val(json.data.isYuan);
				model = et.template("donationpay.html");
				templateObj.config("escape", false);
				modelRender = templateObj.compile(model);
				et.json(et.url("act/nopublic/getAmount"), function(json) {
					if (et.jsonDecode.hasErr(json)) {
						et.jsonDecode.showErr(json);
					} else {
						$(modelRender(json)).appendTo($(".chiose-money"));
