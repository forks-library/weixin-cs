...
function I($name,$default='',$filter=null,$datas=null) {
    ...
    }elseif(isset($input[PARAM_KEY])){
    	$param = $input[PARAM_KEY];
 ...
 * 阿里云图片读取专有方法
 */
function Ai($img, $param = "") {
	$url = "";
	
	if (!empty($img)) {
		$url .= GLOBAL_IMG_SERVER."/";
...
function Ei($id = "", $token = "", $fromUser = "", $connect = false) {
	$str = "";
 ...
 * @return string
 * 
 * 2015-12-12 修正U的写法 严正
 */
function U($url='',$vars='',$suffix=true,$domain=false) {
	$fUrl = '/';
	
	if (defined('SERVER_RESOURCE')) {
		$fUrl = "http://".SERVER_NAME;
	} else {
		$fUrl = "http://".SERVER_NAME.__ROOT__;
	}
	
	if (defined('SERVER_PARTNAME')) {
		$fUrl .= '/'.SERVER_PARTNAME;
	}
    ...
    if (strpos($url, "?") !== false) { // 解析地址里面参数 合并到vars
	...
	if (!empty($url)) {
		...
		$pend = strripos($url, '/');
		if ($pend !== false && $pend == strlen($url) - strlen('/')) {
			$url = substr($url, 0 , strlen($url) - 1);
		}
		
		if (!empty(SERVER_PARTNAME)) {
			$p = strpos($url, SERVER_PARTNAME);
			if ($p !== false) {
				$url = substr($url, $p + strlen(SERVER_PARTNAME));
			}
		}
		...
		if ($pos !== false) {
			$arrUrlPart = explode("/", $url);
			$arrIndex = !empty($arrUrlPart) && is_array($arrUrlPart) ? sizeof($arrUrlPart) : 0;
...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        // 启动session
        if(!isset($_SESSION) && C('SESSION_AUTO_START'))  session_start();
    }elseif('' === $value){ 
        if(''===$name){
            // 获取全部的session
...
function str_startWith($str, $strMark) {
	$str = trim($str);
	if (!empty($str)) {
...
function str_endWith($str, $strMark) {
	$str = trim($str);
	if (!empty($str)) {
