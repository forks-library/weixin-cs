...
class WxVolunteerApplyModel extends VersionModel {
	
	public function addVolunteer($userId, $no, $applicantUnit, $contacts, $phone, $note, $aboutDataImg) {
		$blnSucc  = false;
		$version  = "";
		...
		$dbData["volunteer_no"]        = $this->generateVolunteerNo();
//		$dbData["volunteer_status"] = "1";
//		$dbData["is_message"]       = "1";
		...
		try {
			...
			if (!empty($activityPic) && is_array($activityPic)) {
				$bln = $picModel->addVolunteerPic($dbData["volunteer_no"], $activityPic);
			} else {
				$bln = false;
			}
	...
	protected function generateVolunteerNo() {
		$code = "990".$this->generateRandomTime(true, 10);
		
		if (!empty($code)) {
			try {
				$where = " c.volunteer_no = '".$code."' ";
				$count = $this->where($where)->count();
				
				if (intval($count) > 0) {
					$code = $this->generateVolunteerNo();
				}
			} catch (\Exception $e) {
				$code = "";
			}
