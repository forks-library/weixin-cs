...
//			$name   = EncodeUtil::dbUnicodeEncode(EncodeUtil::unicodeEncode($searchText));
		...
		if (!empty($orderType)) {
			if (intval($orderType) === 1) {
				$order = "c.seq desc, c.already_money desc, c.audit_time desc";
			} else if (intval($orderType) === 2) {
				$order = "c.seq desc, c.audit_time desc";
			} else {
				$order = "c.".$this->getPk();
			}
		} else {
	...
	public function getHelpInformationDetail($id) {
		...
		if (!empty($id)) {
			$where = " c.help_code = '".$id."' and c.status in (3, 5) ";
			...
			if (!empty($data) && is_array($data)) {
				...
				$helpMation["situation"]    = !empty($data["situation_explain"]) ? StringUtil::dbToHtml(EncodeUtil::unicodeDecode(EncodeUtil::dbUnicodeDecode($data["situation_explain"]))) : "";
...
//		$userId, $helpType, $helpNm, $helpId, $helpAddr, $handy, $reason, $medicalExpense, $socialSecurity, $otherRecipients, $statusStatement, $town, $idcardImg, $aboutDataImg
	...
	public function getProjectDetail($infoNo) {
		$projectDetail = array();
		if (!empty($infoNo)) {
			$where = " 1 = 1 and c.help_code = '".$infoNo."' and c.status = 5 ";
			
			$field  = array();
			$field[]= " c.help_title ";
			$field[]= " c.top_pic ";
			$field[]= " c.situation_explain ";
			...
			if (!empty($data) && is_array($data)) {
				...
				$projectDetail["projectTitle"] = EncodeUtil::unicodeDecode(EncodeUtil::dbUnicodeDecode($data["help_title"]));
				$projectDetail["topPic"]       = ImgUtil::imgHandy(urldecode($data["top_pic"]));
				$projectDetail["startTime"]    = !empty($data["start_time"]) ? date("Y-m-d", $data["start_time"]) : date("Y-m-d", time());
				$projectDetail["explain"]      = EncodeUtil::unicodeDecode(EncodeUtil::dbUnicodeDecode($data["situation_explain"]));
				$projectDetail["amountList"]   = $mtAmountControlModel->getAmountList();
			}
		}
	...
	public function getIsYuan($no) {
		$isYuan = array();
		
		$where  = ' 1 = 1 and c.help_code = \''.$no.'\'';
