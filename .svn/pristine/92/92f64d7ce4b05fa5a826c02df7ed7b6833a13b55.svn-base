...
use Com\Leskymob\Manage\OptionManage;
/**
 * CX标签库解析类
 */
class Cx extends TagLib {
    ...
    protected $tags   =  array(
        ...
        'search'    =>  array('attr'=>'textname,placeholder,formid,formethod,buttons', 'level'=>3)
        );
	...
	public function _search($tag, $content) {
		...
		$buttons = isset($tag["buttons"]) && !empty($tag["buttons"]) ? $tag["buttons"] : 'btn_add,btn_exp,btn_imp';
		$formid = isset($tag["formid"]) && !empty($tag["formid"]) ? $tag["formid"] : 'search_form';
		$formethod = isset($tag["formethod"]) && !empty($tag["formethod"]) ? $tag["formethod"] : 'post';
		
		if (!empty($buttons) && (strtolower($buttons) === 'none' || strtolower($buttons) === 'null')) {
			$buttons = '';
		}
		if (strpbrk($buttons, ',') !== false) {
			$buttons = explode(',', $buttons);
		}
		
		// 类别查询 select
//		$selectTag = OptionManage::options("DD_SERACH_TYPE");
//		$tempSelect = '<div id="serach_paraType" class="select-group"><div class="select select-close">';
//		$tempSelect .= '<span class="select-display" value=""></span>'; 
//		$tempSelect .= '<input type="hidden" name="serach_paraType" value="">';
//		$tempSelect .= '<div class="select-img float-right">';
//		$tempSelect .= '<img src="__PUBLIC__/Images/caret-right-b.png"></div></div>';
//		$tempSelect .= '<div class="select-info display-none">';
//		$tempSelect .= '<ul>';
//		if (!empty($selectTag)) {
//			foreach ($selectTag as $key => $val) {
//				$tempSelect .= '<li value="'.$key.'">'.$val.'</li>';
//			}
//		} else {
//			$tempSelect .= '<li value="">请选择</li>';
//		}
//		$tempSelect .= '</ul></div></div>';
		
		$parseStr .= '<div class="operation" id="'.$formid.'">';
...
//		$parseStr .= $tempSelect;
		 ...
		 */
		$tempInput = '';
		$inputTag  = array("name" => $textname, "extend" => "placeholder='".$placeholder."'", "class" => "input");
		$tempInput .= $html->_input($inputTag, '');
		
		$parseStr .= '<div class="input-group float-left">';
		$parseStr .= $tempInput;
		
		/**
		 * 搜索明细按钮
		 */
		$imageBtn = array("id" => "btn_search_list", "class" => "button disabled");
		$tempImage = $html->_imageBtn($imageBtn);
		$parseStr .= $tempImage;
		$parseStr .= '</div>';
		 ...
		 * 搜索按钮
		 */
		$imageBtn = array("id" => "search_b", "class" => "button btn_search float-left", "text" => "检索");
		$tempImage = $html->_imageBtn($imageBtn);
		...
		if (!empty($buttons)) {
			if (is_array($buttons)) {
				foreach ($buttons as $button) {
					$imageBtn 	= array("id" => trim($button), "class" => "button");
					$tempButton = $html->_imageBtn($imageBtn);
					$temp 		= $this->createImpExpTags($tempButton, $button);
					$btnStr 	.= $temp;
				}
			} else if (is_string($buttons)) {
				$imageBtn 	= array("id" => trim($buttons), "class" => "button");
				$tempButton = $html->_imageBtn($imageBtn);
				$temp 		= $this->createImpExpTags($tempButton, $buttons);
				$btnStr 	.= $temp;
			}
		}
		if (!empty($btnStr)) {
			$btnStr = '<div class="float-right">'.$btnStr.'</div>';
		}
	...
	private function createImpExpTags ($tempButton, $classType) {
		$temp = "";
		if ($classType == "btn_exp" || $classType == "btn_imp") {
			if ($classType == "btn_exp") {
				$temp = '<div class="btn-dropdown-group">';
				$temp .= $tempButton;
				$temp .= '<div class="btn-dropdown-list btn-exp-list display-none">';
				$temp .= '<ul>';
				$temp .= '<li class="opc8 btn-dropdown-item btn_exp_current">导出当前</li>';
				$temp .= '<li class="opc8 btn-dropdown-item btn_exp_all">导出全部</li>';
				$temp .= '</ul>';
				$temp .= '</div></div>';
			}
			if ($classType == "btn_imp") {
				$temp = '<div class="btn-dropdown-group">';
				$temp .= $tempButton;
				$temp .= '<div class="btn-dropdown-list btn-imp-list display-none">';
				$temp .= '<ul>';
				$temp .= '<li class="opc8 btn-dropdown-item btn_imp_tmp">模板下载</li>';
				$temp .= '<li class="opc8 btn-dropdown-item btn_imp">数据导入</li>';
				$temp .= '</ul>';
				$temp .= '</div></div>';
			}
		} else {
    ...
    public function _import($tag,$content,$isFile=false,$type='') {
        ...
        if($isFile) {
            ...
            $basepath   =   !empty($tag['basepath']) ? $tag['basepath'] : (defined('SERVER_RESOURCE') ? SERVER_RESOURCE.'/Public' : __ROOT__.'/Public');
            // 命名空间方式导入外部文件
            $array      =   explode(',',$file);
            foreach ($array as $val){
                if(strpos ($val, '?')) {
