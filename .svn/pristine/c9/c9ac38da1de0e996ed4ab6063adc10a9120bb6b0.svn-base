require(["weixin-normal", "page"], function(wx) {
	et.fnWxShare = function() {
		...
		wx.ready(function() {
			...
			var imgUrl = 'http://mor-eyoo.img-cn-shenzhen.aliyuncs.com/mor/101020171221174912187854.png';
		...
		if (!et.isNull(window._wxAuthUrl)) {
			et.json(et.url("wxint/jsauth/apiKey"), function(json) {
				if (et.jsonDecode.hasErr(json)) {
					...
					window._wxShrTitle = !et.isNull(window._wxShrTitle) ? window._wxShrTitle + ' 东莞慈善 ' : '东莞慈善';
					window._wxShrDesc  = window._wxShrDesc || '东莞慈善邀你一起做公益';
