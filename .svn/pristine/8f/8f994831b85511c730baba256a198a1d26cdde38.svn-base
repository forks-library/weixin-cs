...
//        }
     ...
     * @return mixed
     */
    public function getField($field,$sepa=null,$isSuper = false) {
    	$isFieldArray = false;
    	if (is_string($field)) {
    		$p = strrpos($field, 'AS tp_');
	    	if ($p === false) {
	    		$field .= ", version";
	    	}
        ...
        if(strpos($field,',') && false !== $sepa) { // 多字段
            ...
            if ($isSuper) {
            	$resultSet          =   $this->db->select($options, false);
            } else {
            	$resultSet          =   $this->db->select($options);
            }
...
//                }
				for ($i = 0; $i < sizeof($resultSet); $i++) {
					$result = $resultSet[$i];
					$cols[$i + 1]   =  is_string($sepa)?implode($sepa,array_slice($result,1)):$result;
				}
                if(isset($cache)){
                    S($key,$cols,$cache);
                }
            ...
            if(true !== $sepa) {// 当sepa指定为true的时候 返回所有数据
           	...
           	$p = strrpos($field, 'AS tp_');
           	if ($p !== false) {
           		$result = $this->db->select($options, false);
           	} else if ($isSuper) {
           		$result = $this->db->select($options, false);
           	} else {
